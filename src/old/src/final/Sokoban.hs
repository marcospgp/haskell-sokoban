...
-- Note: All Y coordinates used in the code are relative to the top of the map
data World = World {
...
mapHeight = 30 :: Int
spriteWidth = (fromIntegral windowWidth) / (fromIntegral mapWidth) :: Float -- Used to calculate sprite positions when rendering the map
...
drawWorld world = pictures (drawnPlayer : (drawnMap ++ drawnBoxes))
    where
        -- Sprites
        playerSprite =    Scale spriteWidth spriteHeight (unsafePerformIO $ loadBMP "./assets/Character1.bmp")
        boxSprite =       Scale spriteWidth spriteHeight (unsafePerformIO $ loadBMP "./assets/Crate_Brown.bmp")
        -- Sprite that represents a box which has already been placed on one of the designated spots
        placedboxSprite = Scale spriteWidth spriteHeight (unsafePerformIO $ loadBMP "./assets/Crate_Blue.bmp")
        groundSprite =    Scale spriteWidth spriteHeight (unsafePerformIO $ loadBMP "./assets/Ground_Concrete.bmp")
        -- Sprite that represents a place on the ground where a box should be placed
        boxGroundSprite = Scale spriteWidth spriteHeight (unsafePerformIO $ loadBMP "./assets/Ground_Sand.bmp")
        wallSprite =      Scale spriteWidth spriteHeight (unsafePerformIO $ loadBMP "./assets/Wall_Beige.bmp")
...
drawMap world wallSprite groundSprite boxGroundSprite = aux world 0 0
    where
        ...
        aux world xPos yPos = (drawMapLine x xPos yPos) ++ (aux (world {mapLines = xs}) 0 (yPos + 1))
            where
                (x : xs) = mapLines world

        drawMapLine [] _ _ = []
        drawMapLine (x : xs) xPos yPos = (Translate (spriteWidth * xPos) (spriteHeight * yPos) spriteToUse) : (drawMapLine xs (xPos + 1) yPos)
            where
...
drawPlayer world sprite = Translate (spriteWidth * playerX) (spriteHeight * playerY) sprite
    where
...
drawBoxes (World _ _ []) _ _ = []
drawBoxes world@(World mapLines playerPos (x : xs)) boxSprite placedBoxSprite =
    (Translate (spriteWidth * (fromIntegral boxX)) (spriteHeight * (fromIntegral boxY)) spriteToUse) : (drawBoxes (world {boxPos = xs}) boxSprite placedBoxSprite)
        where
            boxX = x !! 0
            boxY = x !! 1
...
updateWorld deltaT world = world -- Quietly do nothing (my life)
